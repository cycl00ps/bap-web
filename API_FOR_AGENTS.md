...
Browser sessions are supported, but agents should use API tokens generated by a user or administrator.

1. A user creates a token from the API Tokens page, or through the API with a browser session. Admins can create tokens for any existing user. Non-admin users can create only their own non-admin tokens.
...
{"name":"agent","is_admin":false}
```

The response includes `secret` once. Store it securely. If `expires_at` is omitted, BAP Web sets it to 30 days from creation.

```json
{"token":{"id":"...","name":"agent","prefix":"bap_...","is_admin":false,"owner_username":"admin"},"secret":"bap_..."}
```
...
Non-admin bearer tokens cannot create additional API tokens. Admin bearer tokens can create tokens, but should still use the least privilege and shortest practical expiry.
...
Admin users can list and revoke all tokens. Non-admin users can list and revoke only their own tokens.
